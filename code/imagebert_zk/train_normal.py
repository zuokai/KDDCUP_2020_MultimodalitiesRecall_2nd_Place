...
tf.app.flags.DEFINE_string('pretrained_model_path', 'models/model_attention_kdd_am.ckpt-200051', '')
...
def tower_loss(num_boxes, np_boxes_5, np_images_features, np_idx_class_labels, np_len_class_labels,
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):
        model_loss, probs, loss_list, probs_all = model.model_attention_channel_e(num_boxes, np_boxes_5, np_images_features, np_idx_class_labels, np_len_class_labels,
    ...
    #num_labels = tf.cast(tf.size(weight_label_query), tf.float32) / tf.reduce_sum(weight_label_query)
    #acc = tf.reduce_mean(tf.cast(tf.equal(preds, label_query), dtype=tf.float32) * weight_label_query) * num_labels
    #acc = tf.reduce_mean(tf.cast(tf.equal(preds, label_list_), dtype=tf.float32))
    acc = tf.cast(tf.equal(preds, label_list_), dtype=tf.float32)
    total_loss = tf.add_n([model_loss] + tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES))

    if reuse_variables is None:
        tf.summary.scalar('total_loss', total_loss)

    return total_loss, model_loss, acc, probs, loss_list, probs_all
...
def main(argv=None):
    ...
    for i, gpu_id in enumerate(gpus):
        with tf.device('/gpu:%d' % gpu_id):
            with tf.name_scope('model_%d' % gpu_id) as scope:
                ...
                total_loss, model_loss, acc, probs, loss_list, probs_query = tower_loss(num_boxes_, np_boxes_5_, np_images_features_,
    ...
    #grads = [(tf.clip_by_norm(grad, 1.), var) for grad, var in grads]
    ...
    #var_to_restore = [v for v in all_vars if not 'kdd' in v.name]
    ...
    #var_to_restore = [v for v in var_to_restore if not 'am_kernel' in v.name]
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for step in range(FLAGS.max_steps):
            ...
            _, ml, tl, acc_, probs_, loss_list_, probs_all_ = sess.run([train_op, model_loss, total_loss, acc, probs, loss_list[0], probs_query], feed_dict={num_boxes_list: data[3],
                                                                              np_boxes_5: data[4],
            ...
            print("loss: ", ml, ", acc: ", acc_, " product_id: ", data[0])
            '''
            ...
            for eval_index in range(batch_size):
                file_ob.write(str(data[9][eval_index]) + "\t" + str(data[0][eval_index]) + "\t" + str(
                    probs_[eval_index][1]) + "\t")
                #print("shape: ", probs_all_.shape)
                for prob in probs_all_[eval_index, :]:
                    file_ob.write(str(prob) + ",")
                file_ob.write("\n")
