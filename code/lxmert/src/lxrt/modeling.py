...
class VisualConfig(object):
    ...
    def __init__(self,
        ...
        self.boxes_label_dim = 768
        self.boxes_labeltext_dim = MAX_LABELTEXT_LENGTH
...
class BertCrossattLayer(nn.Module):
    def __init__(self, config):
        super(BertCrossattLayer, self).__init__()
...
class LXRTXLayer(nn.Module):
    def __init__(self, config):
        super(LXRTXLayer, self).__init__()
...
class VisualFeatEncoder(nn.Module):
    def __init__(self, config):
        ...
        label_dim = VISUAL_CONFIG.boxes_label_dim
        boxes_labeltext_dim = VISUAL_CONFIG.boxes_labeltext_dim
        ...
        # Box label encoding
        self.label_conv = nn.Conv2d(boxes_labeltext_dim, 1, kernel_size=1)
        self.label_fc = nn.Linear(label_dim, config.hidden_size)
        self.label_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)

        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, visn_input, boxes_label_lang_feats):
        ...
        z = self.label_conv(boxes_label_lang_feats.permute(0, 2, 1, 3))
        z = z.squeeze()
        z = self.label_fc(z)
        z = self.label_layer_norm(z)
        output = (x + y + z) / 3
...
class LXRTEncoder(nn.Module):
    def __init__(self, config):
        ...
        # self.num_b_layers = VISUAL_CONFIG.b_layers # box cross feature
        # print("LXRT encoder with %d l_layers, %d x_layers, %d r_layers, and %d b_layers." %
        #       (self.num_l_layers, self.num_x_layers, self.num_r_layers, self.num_b_layers))
        ...
        #     [BertLayer(config) for _ in range(self.num_b_layers)]
        # )

    def forward(self, lang_feats, boxes_label_lang_feats, lang_attention_mask, boxes_label_lang_feats_mask, 
                visn_feats, visn_attention_mask=None):
        # Run visual embedding layer
        # Note: Word embedding layer was executed outside this module.
        #       Keep this design to allow loading BERT weights.

        visn_feats = self.visn_fc(visn_feats, boxes_label_lang_feats)

        # Run language layers
        for layer_module in self.layer:
            lang_feats = layer_module(lang_feats, lang_attention_mask)
            # boxes_label_lang_feats = layer_module(boxes_label_lang_feats, boxes_label_lang_feats_mask)
        ...
        # for layer_module in self.b_layers:
        #     boxex_lable_lang_feats, visn_feats = layer_module(boxex_lable_lang_feats, boxes_label_lang_feats_mask,
        #                                           visn_feats, visn_attention_mask)
...
class LXRTModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, boxes_label_input_ids, 
                ...
                boxes_label_token_type_ids=None, boxes_label_attention_mask=None, 
                visual_feats=None, visual_attention_mask=None):
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)
        if boxes_label_attention_mask is None:
            boxes_label_attention_mask = torch.ones_like(boxes_label_input_ids)
        if boxes_label_token_type_ids is None:
            boxes_label_token_type_ids = torch.zeros_like(boxes_label_input_ids)
        ...
        extended_boxes_label_attention_mask = boxes_label_attention_mask.unsqueeze(1).unsqueeze(2)
        extended_boxes_label_attention_mask = extended_boxes_label_attention_mask.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
        extended_boxes_label_attention_mask = (1.0 - extended_boxes_label_attention_mask) * -10000.0
        ...
        boxes_label_embedding_output = torch.stack([self.embeddings(boxes_label_input_ids[i,:,:], boxes_label_token_type_ids[i,:,:]) for i in range(len(boxes_label_input_ids))], 0)

        # Run LXRT backbone
        lang_feats, visn_feats = self.encoder(
            embedding_output,
            boxes_label_embedding_output,
            extended_attention_mask,
            extended_boxes_label_attention_mask, 
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    def __init__(self, config, mode='lxr'):
        ...
        """
        super(LXRTFeatureExtraction, self).__init__(config)
        self.config = config
        self.bert = LXRTModel(config)
        self.mode = mode
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, boxes_label_input_ids, 
        ...
        feat_seq, pooled_output = self.bert(input_ids, boxes_label_input_ids, 
                                            token_type_ids, attention_mask,
                                            boxes_label_token_type_ids, boxes_label_attention_mask,
