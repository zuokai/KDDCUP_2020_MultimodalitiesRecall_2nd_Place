...
def main(_):
  batch_size = 30
  modelcheckpoint = '../ModelCheckPointGPUSMSLOSS/ImageBertKDD.ckpt-85001'
  ...
  train_op,total_loss,next_sentence_loss,next_sentence_accuracy,next_sentence_accuracy_op,next_sentence_prob,query_id,product_id\
        = bertmodel(bert_config=bert_config,bert_init_checkpoint=FLAGS.init_checkpoint,learning_rate=FLAGS.learning_rate,num_train_steps=FLAGS.num_train_steps,num_warmup_steps=FLAGS.num_warmup_steps,use_one_hot_embeddings=False,features=features,is_training=True,ngpus=1)
  ...
  with tf.Session(config=config) as sess:
    ...
    for val in result.values():
      sumres += len(val)
    assert sumres == 28830
    with open("submission_con85k.csv","wb") as file:
      line = "query-id,product1,product2,product3,product4,product5"
      file.write(line+'\n')
      for key in sorted(result.keys()):
        value = result[key]
        productids=map(lambda x:x[0],sorted(value,key=lambda x:x[1],reverse=True)[:5])
        key_values = [str(key)]+productids
        line = ','.join(map(str,key_values))
        file.write(line+'\n')
